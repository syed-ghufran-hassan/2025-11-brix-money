...
/**
 * @title 02_DeployProtocol
 * @notice [HUB] Deploys protocol layer for contracts-v2: FastAccessVault, YieldForwarder, iTryIssuer, StakediTryCrosschain
 * @dev Depends on: 01_DeployCore (needs DLF, iTry, Oracle, Custodian addresses)
 *
 * Key changes from v1:
 ...
 * - StakediTryCrosschain (with COMPOSER_ROLE support) instead of StakedUSDeV2
 * - Pre-computes addresses to solve chicken-egg dependency (FastAccessVault needs iTryIssuer address)
 *
 * Deployment sequence: 01 → 02 → 03
 */
contract DeployProtocol is Script, DeploymentRegistry {
    ...
    function run() public {
        console2.log("=========================================");
        console2.log("02_DeployProtocol: Starting Protocol Deployment (v2)");
        console2.log("=========================================");
        ...
        require(registryExists(), "Core contracts not deployed. Run 01_DeployCore first.");
        ...
        console2.log("  StakediTryCrosschain:", predictedStaking);
        ...
        StakediTryCrosschain staking = _deployStaking(factory, addrs.itryToken);
        ...
        // Grant REWARDER_ROLE to iTryIssuer on StakediTryCrosschain
        staking.grantRole(REWARDER_ROLE, address(issuer));
        console2.log("  Granted REWARDER_ROLE to iTryIssuer on StakediTryCrosschain");
    ...
    function _deployStaking(Create2Factory factory, address itryToken) internal returns (StakediTryCrosschain) {
        return StakediTryCrosschain(_deployDeterministic(factory, _getStakingBytecode(itryToken), STAKING_SALT, "StakediTryCrosschain"));
    }
    ...
    function _getStakingBytecode(address itryToken) internal view returns (bytes memory) {
        return abi.encodePacked(
            type(StakediTryCrosschain).creationCode,
            abi.encode(
