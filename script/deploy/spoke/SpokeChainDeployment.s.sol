 ...
 * - UnstakeMessenger added for crosschain unstaking (Phase 6-7)
 *
 * Deployment sequence: 01 → 02 → 03 (hub) then 20 (spoke)
 * Next steps: Configure peers on hub chain
 *
 * Usage:
 ...
 *   - VAULT_COMPOSER: Address of VaultComposer on hub chain (required, for UnstakeMessenger)
 */
contract DeploySpoke is Script {
    ...
    struct DeployedContracts {
        ...
        wiTryOFT wiTryOFT;
        UnstakeMessenger unstakeMessenger;
    }
    ...
    function run() public {
        ...
        console2.log("Hub Vault Composer:", config.hubVaultComposer);
    ...
    function _loadConfig() internal returns (SpokeDeploymentConfig memory config) {
        ...
        address hubVaultComposer = vm.envOr("VAULT_COMPOSER", address(0));

        // If environment variables not set, use defaults for testnet
        if(endpoint == address(0)) {
            console2.log("WARNING: Using default testnet configuration");
            console2.log("Set SPOKE_CHAIN_ENDPOINT, HUB_CHAIN_EID, HUB_ITRY_ADAPTER, HUB_SHARE_ADAPTER, VAULT_COMPOSER for production");
            console2.log("");

            endpoint = OP_SEPOLIA_ENDPOINT;
            hubChainEid = SEPOLIA_EID;
            // These need to be set from hub deployment (03_DeployCrossChain)
            ...
            require(hubVaultComposer != address(0), "VAULT_COMPOSER must be set");
        }
        ...
        config = SpokeDeploymentConfig({
            ...
            hubVaultComposer: hubVaultComposer,
    ...
    function _validateConfig(SpokeDeploymentConfig memory config) internal pure {
        ...
        require(config.hubVaultComposer != address(0), "Invalid hub vault composer address");
        ...
        returns (DeployedContracts memory contracts)
    {
        ...
        // Deploy UnstakeMessenger (Phase 6-7: Crosschain Unstaking)
        contracts.unstakeMessenger = new UnstakeMessenger(
            config.endpoint,
            config.deployer,
            config.hubChainEid
        );
        console2.log("UnstakeMessenger deployed:", address(contracts.unstakeMessenger));
    ...
    function _logDeploymentSummary(SpokeDeploymentConfig memory config) internal view {
        ...
        console2.log("  UnstakeMessenger:", address(deployed.unstakeMessenger));
        console2.log("");
        ...
        console2.log("1. Configure hub chain peers (manual or script):");
        console2.log("   - Hub iTRY Adapter -> Spoke iTRY OFT");
        console2.log("   - Hub wiTRY Adapter -> Spoke wiTRY OFT");
        console2.log("   - Hub Vault Composer -> Spoke Unstake Messenger");
        console2.log("");
        console2.log("2. Set LayerZero libraries (if needed)");
        console2.log("");
        ...
        console2.log("  UnstakeMessenger:", _toBytes32String(address(deployed.unstakeMessenger)));
        console2.log("=========================================");
    }
    ...
    function getDeployedContracts() external view returns (
        address itryTokenOFT,
        address wiTryOFT,
        address unstakeMessenger
    ) {
        return (
            address(deployed.itryTokenOFT),
            address(deployed.wiTryOFT),
            address(deployed.unstakeMessenger)
        );
    }
}
