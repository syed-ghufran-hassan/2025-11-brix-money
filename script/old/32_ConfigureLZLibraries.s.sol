...
/**
 * @title 32_ConfigureLZLibraries
 * @notice [HUB & SPOKE] Configures send and receive libraries for LayerZero OApp contracts
 * @dev This script must be run on BOTH chains (hub and spoke) to complete configuration
 *
 * Key changes from v1:
 * - Updated script numbering (32 in sequence)
 * - Same functionality, different naming convention
 * - Auto-detects chain and configures accordingly
 *
 * Deployment sequence: 10 → 11 → 12 (hub) → 20 (spoke) → 30 (peers) → 32 (libraries)
 * Next steps: 40_VerifyHub → 41_VerifySpoke → 42_VerifyBridge
 *
 * Prerequisites:
 *   1. Hub deployment complete (12_DeployCrossChain)
 *   2. Spoke deployment complete (20_DeploySpoke)
 *   3. Peer configuration complete (30_ConfigurePeers)
 *
 ...
 *   forge script script/32_ConfigureLZLibraries.s.sol:ConfigureLZLibraries \
 ...
 *   - HUB_ITRY_ADAPTER: Address of iTryTokenOFTAdapter on Sepolia (for hub config)
 *   - SPOKE_ITRY_OFT: Address of iTryTokenOFT on OP Sepolia (for spoke config)
 *
 * The script auto-detects which chain it's running on and configures accordingly.
 */
contract ConfigureLZLibraries is Script {
    ...
    function run() public {
        console2.log("=========================================");
        console2.log("32_ConfigureLZLibraries: LayerZero Library Setup");
        console2.log("=========================================");
        console2.log("");
        ...
        console2.log("3. Verify deployments with 40_VerifyHub.s.sol and 41_VerifySpoke.s.sol");
        console2.log("4. Test cross-chain bridging with 42_VerifyBridge.s.sol");
        console2.log("=========================================");
    }
    ...
    function _configureHubChain() internal {
        ...
        // Set send library for messages going to OP Sepolia
        console2.log("Setting send library...");
        console2.log("  Library:", SEPOLIA_SEND_LIB);
        (bool success,) = SEPOLIA_ENDPOINT.call(
            abi.encodeWithSignature(
                "setSendLibrary(address,uint32,address)", hubAdapter, OP_SEPOLIA_EID, SEPOLIA_SEND_LIB
            )
        );
        ...
        // Set receive library for messages coming from OP Sepolia
        console2.log("Setting receive library...");
        console2.log("  Library:", SEPOLIA_RECEIVE_LIB);
        ...
        require(success, "Failed to set receive library");
    ...
    function _configureSpokeChain() internal {
        ...
        // Set send library for messages going to Sepolia
        console2.log("Setting send library...");
        console2.log("  Library:", OP_SEPOLIA_SEND_LIB);
        (bool success,) = OP_SEPOLIA_ENDPOINT.call(
            abi.encodeWithSignature(
                "setSendLibrary(address,uint32,address)", spokeOFT, SEPOLIA_EID, OP_SEPOLIA_SEND_LIB
            )
        );
        require(success, "Failed to set send library");
        console2.log("  [OK] Send library configured");
        ...
        // Set receive library for messages coming from Sepolia
        console2.log("Setting receive library...");
        console2.log("  Library:", OP_SEPOLIA_RECEIVE_LIB);
